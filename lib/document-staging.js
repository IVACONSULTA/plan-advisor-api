...
async function promoteStagingToProfile(profileSlug, country_id, provider_id, profile_id, userId) {
  ...
  for (const row of rows) {
    try {
      console.log(`[promoteStagingToProfile] Processing staging doc ${row.id}: ${row.filename}`);
      console.log(`[promoteStagingToProfile]   Staging path: ${row.storage_path}`);
      
      const buf = await readDocument(row.storage_path);
      console.log(`[promoteStagingToProfile]   Read ${buf.length} bytes from staging`);
      ...
      console.log(`[promoteStagingToProfile]   Saved to promoted path: ${storagePath}`);
      ...
      console.log(`[promoteStagingToProfile]   Created documents row: ${ins.rows[0].id}`);

      await deleteDocument(row.storage_path);
      console.log(`[promoteStagingToProfile]   Deleted staging file`);
      
      await db.query(`DELETE FROM document_staging WHERE id = $1`, [row.id]);
      console.log(`[promoteStagingToProfile]   Deleted staging DB row`);
      ...
      console.error(`[promoteStagingToProfile] Failed to promote ${row.id}:`, promoteErr);
      throw new Error(`Failed to promote staging doc ${row.filename}: ${promoteErr.message}`);
    }
  }
