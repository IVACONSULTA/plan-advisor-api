...
async function runDocumentAnalysis({ userId, profile_id, document_ids, message = '', existingAnalysisId = null }) {
  const { rows: docs } = await db.query(
    `SELECT d.*, cp.country_id, cp.provider_id, cp.calculation_basis, cp.notes,
  ...
  const profileCalculationBasis = docs[0].calculation_basis;
  const profileNotes = docs[0].notes;

  console.log(`[runDocumentAnalysis] Profile metadata:`, {
    calculation_basis: profileCalculationBasis,
    has_notes: Boolean(profileNotes),
  });
    ...
    const { rowCount } = await db.query(
      `INSERT INTO transaction_rules
         (profile_id, input_key, label, direction, obligation, operation_group,
          ...
          confidence, status, ai_proposed_value, calculation_basis, notes)
