...
async function runDocumentAnalysis({ userId, profile_id, document_ids, message = '', existingAnalysisId = null }) {
  ...
  try {
    ...
    agentResponse = data;
    logDocumentAgentResponse('Doc agent HTTP 200 response', agentResponse);
  } catch (agentErr) {
  ...
  if (existingAnalysisId) {
    // Async flow: finalize the pre-created 'running' row.
    ...
    // Sync flow (legacy): insert a fresh row.
    const { rows: analysis } = await db.query(
  ...
  // Normalise response keys: agent may return `transaction_rules` (new) or `rules` (legacy/stub).
  ...
  // Build filename → document UUID map for source_document_id resolution.
  // Stored filenames may have a timestamp prefix (e.g. `1a2b3c_original.pdf`), so we
  // also try a suffix match against the original filename the agent reports.
  const docByFilename = {};
  ...
  function resolveSourceDocId(sourceExcerpt) {
    ...
    // Suffix / contains match for prefixed stored filenames.
  ...
  if (rulesArray.length) {
    console.log(`[runDocumentAnalysis] Persisting ${rulesArray.length} transaction rule(s) for profile ${profile_id}`);
  ...
  if (plansArray.length) {
    console.log(`[runDocumentAnalysis] Persisting ${plansArray.length} plan(s) for profile ${profile_id}`);
  ...
  if (assumptionsArray.length) {
    console.log(`[runDocumentAnalysis] Persisting ${assumptionsArray.length} assumption(s) for profile ${profile_id}`);
